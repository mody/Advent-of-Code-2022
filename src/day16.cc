...
void part1(Data const& base_data)
{
    ...
    assert(key_to_index("AA") == 0);

    do {
        unsigned score = 0u, length = 1;
        int minutes = 30;
        std::string from = "AA";
        for (std::string to : path) {
            const Index i = key_to_index(from);
            const Index j = key_to_index(to);
            ++length;
            minutes -= distances[i][j];
            if (minutes < 1) {
                break;
            }
            score += base_data.at(to).rate * --minutes;
            from = std::move(to);
        }
